...
状态为 `failed` 表示管理记录对应的签发任务失败，不等于已有证书被 CA 撤销。首次签发失败时通常没有任何已发布文件，可以修正配置后“重试签发”，也可以“删除失败记录”；删除管理记录后，原任务和审计历史仍会保留。只有已经成功签发且仍有效的公共证书才适用“撤销”，该操作会通知 CA 吊销证书，不能代替删除记录。
...
- **如何删除 `failed` 记录**：进入证书详情点击“删除失败记录”。首次签发失败且没有文件时，页面只删除管理记录；若失败发生在已有证书的续签阶段且仍有已发布文件，可选择先备份再删除文件。任务与审计历史不会被删除。
